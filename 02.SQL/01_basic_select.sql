...
-- EMP 테이블의 모든 컬럼의 모든 항목을 조회. -- *: 모든컬럼
select * from emp;
...
- 같은 컬럼을 여러번 조회할 수 있다.
************************************** */
select 10,20; -- select절에 상수값을 넣어서 확인. 이때 from 생략
select '안녕하세요' as 인사말;
select '2020-03-25' as "오늘 날짜";
-- 문자열 합치기
...
select  emp_id,
		emp_name
from emp
where emp_name like '%?%%' escape '?' ;-- # 패턴문자가 오면 Iteral
...
select  emp_id
        ...
        hire_date
from emp
where hire_date like '2007%' or hire_date like '2006%' or hire_date like '2005%';
...
select * from emp
where dept_name  in ('Shipping','Executive') and salary >= 6000;
...
select  emp_id,
        ...
        salary
from emp
order by 3 asc, 4 desc;  -- 1차로 job의 오름차순 정렬, 2차로 salary 내림차순 (job이 같은것 안에서 정렬)
