...
select  e.emp_id,
        ...
        e.hire_date,
        d.dept_name
from    emp e inner join dept d on e.dept_id = d.dept_id; -- inner 생략가능
...
select  e.emp_id,
        ...
        e.salary,
        s.grade
from	emp e inner join salary_grade s on e.salary between s.low_sal and s.high_sal;
...
select  avg(e.salary),
		d.dept_name
from emp e inner join dept d on e.dept_id = d.dept_id
group by d.dept_id, d.dept_name -- dept_name이 Unique Key 컬럼이 아님(같은 이름의 다른 부서가 있을 수 있다).
								-- 그래서 dept_id와 같이 group by로 묶어준다
order by 1 desc;
...
from emp e inner join job j on e.job_id = j.job_id
				 join dept d on e.dept_id = d.dept_id
                 join salary_grade s on e.salary between low_sal and high_sal
order by 5 desc;
-- select * from salary_grade;
...
select  s.grade,
        ...
        d.dept_name,
        count(*)
from emp e inner join salary_grade s on e.salary between low_sal and high_sal
				 join dept d on e.dept_id = d.dept_id
where s.grade = 1
...
select  d.dept_id,
		d.dept_name,
        count(e.emp_id) -- count(*) : 행수를 count하기 때문에 직원이 없ㅓ도 1이 나옴
