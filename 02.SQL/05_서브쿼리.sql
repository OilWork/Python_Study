...
select  emp_id,
        ...
        salary
from emp
where job_id = (select job_id from emp where emp_id = 120); -- 서브 쿼리
-- 1. 서브 쿼리가 먼저 실행된다
-- 2. 서브 쿼리의 조회결과를 이용해 메인쿼리가 실행된다.
...
select * from emp
where (job_id, dept_id) = (select job_id, dept_id from emp where emp_id = 115); -- and ===> 값을 직접넣는 경우에는 DBMS마다 가능여부가 다르다
...
having avg(e.salary) >= (select avg(salary) from emp)
order by 2
) t; -- inlineview (from절에 subquery 사용)
...
select  *
from emp
where salary < (select avg(salary) from emp where job_id = 'ST_CLERK') and (job_id != 'ST_CLERK' or job_id is null);
...
select e.* from emp e left join job j on e.job_id = j.job_id
where e.job_id in (select job_id from job where max_salary<=6000);
...
고객(데이터-부모) 주문(내역-자식) -> 특정고객이 주문을 했는지 여부
장비(데이터) 대여(내역) -> 특정 장비가 대여됬는지 여부
공연장(데이터) 대관(내역) -> 특정 공연장이 대관된적이 있는지 여부
************************************************************************************* */
...
where exists(select 1 from orders where cust_id = c.cust_id);

-- TODO: 고객(customers) 중 주문(orders)을 한번도 하지 않은 고객들을 조회.
select * from customers c
where not exists(select 1 from orders where cust_id = c.cust_id);
