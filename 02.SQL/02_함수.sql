 ...
 *************************************************************************************************************** */
select upper('abCdD'), lower('aBdCdE');
select char_length('aaaaaa');
select format(10000000,0); -- 정수의 자리구분 , 를 넣어준다.
select format(1234567.8765, 3); -- (대상값, 소수점 자리수)
select concat('홍길동','님');
select insert('123456789', 2,5, '안녕하세요'); -- 2번째 글자부터 5글자를 '안녕하세요' 변경
select replace('aaaaaaaaabbbbbbbbcccccccaaaaaaccccaaaa','a','가'); -- 'a'를 모두 '가'로 변경해라 여러글자 지정할 수 있다.
select left('123456789',5); -- 대상 문자열을 왼쪽에서 5글자만 반환
select right('123456789',5); -- 대상 문자열을 오른쪽에서 5글자만 반환
select substring('1234567890',4,3); -- 4번 글짜부터 3글자만 반환
select trim('     aaaa      ') as "v"; -- 좌우공백 제거
select ltrim('     aaaa      ') as "v"; -- 왼쪽공백 제거
select rtrim('     aaaa      ') as "v"; -- 오른쪽공백 제거
select trim(both '-' from '--------abcdef-----') as "v"; -- 공백이외의 문자를 제거
select trim(leading '-' from '--------abcdef-----') as "v";
select trim(trailing '-' from '--------abcdef-----') as "v";
select lpad('abcd', 10, ' ') as "v"; -- 10글자의 공간을 채운다. 모자란 것은 공백으로 채운다
select rpad('abcd', 10, ' ') as "v";
use hr;
-- 단일행 함수 -> select절, where절에서 사용 (컬럼에 적용)
-- EMP 테이블에서 직원의 이름(emp_name)을 모두 대문자, 소문자, 이름 글자수를 조회
 select upper(emp_name),
		lower(emp_name),
        char_length(emp_name),
        lpad(emp_name, 11, ' ') as "name"
 from emp;
...
select  round(50.12),-- 정수 위치자리 0
		round(50.56),
        round(50.56789,2); -- 소수점 2번째 자리 아래서 반올 

select truncate(50.12, 0);  -- 내림 지정된 부분 뒤를 버린다 정수 위치지정 필수
...
select  emp_id,
		salary,
        round(salary*1.15) as "SAL_RAISE"
from emp;
...
select datediff(now(), '2021-05-26'); -- now() - 2021/05/26 뺀 날짜수
...
select  emp_name,
		adddate(hire_date, interval -6 month) as "입사 6개월전",
        hire_date,
        adddate(hire_date, interval 6 month) as  "입사 6개월후"
from emp
...
select dayofweek(hire_date) from emp
where emp_id = 100;
...
select coalesce(null, null, null, 3, null);
...
from emp
where job in ('AD_PRES','FI_ACCOUNT','PU-CLERK');
select * from emp;
